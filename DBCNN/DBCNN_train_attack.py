...
class DBCNNManager(object):
    def __init__(self, options, path):
        ...
        self._train_loader = torch.utils.data.DataLoader(
            ...
            shuffle=True, num_workers=8, pin_memory=True) #num_workers=0
        self._test_loader = torch.utils.data.DataLoader(
            test_data, batch_size=1,
            shuffle=False, num_workers=8, pin_memory=True) #num_workers=0
    ...
    def train(self):
        ...
        for t in range(self._options['epochs']):
            ...
            if test_srcc > best_srcc:
                ...
                pwd = os.getcwd()
                if self._options['fc'] == True:
                    # modelpath = os.path.join(pwd,'fc_models',(self._options['dataset'] + '_net_params' + '_best_ori_exp' + str(self._options['expid']) + '.pkl'))
                    modelpath = os.path.join(pwd,'fc_models',(self._options['dataset'] + '_net_params' + '_best_ori_exp' + str(self._options['expid']) + '_seed' + str(self._options['seed']) + '.pkl'))
                else:
                    # modelpath = os.path.join(pwd,'db_models',(self._options['dataset'] + '_net_params' + '_best_ori_exp' + str(self._options['expid']) + '.pkl'))
                    modelpath = os.path.join(pwd,'db_models',(self._options['dataset'] + '_net_params' + '_best_ori_exp' + str(self._options['expid']) + '_seed' + str(self._options['seed']) + '.pkl'))
...
def main():
    ...
    parser.add_argument('--expid', dest='expid', type=int,
                        default=0, help='Exp id for splitting.')
    parser.add_argument('--seed', dest='seed', type=int,
                        default=919, help='Seed for splitting.')
    ...
    path = {
        ...
        'fc_root': os.path.join('./fc_models',(args.dataset + '_net_params' + '_best_ori_exp' + str(options['expid']) + '.pkl')),
        #os.path.join('fc_models','net_params_best.pkl'),
    ...
    if options['dataset'] == 'live':          
        ...
        # index = [22, 6, 3, 16, 11, 7, 28, 17, 14, 8, 5, 29, 21, 25, 27, 26, 19, 15, 1, 23, 2, 4, 18, 24, 13, 9, 20, 10, 12]
